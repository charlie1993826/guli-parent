...
public class RedisUtil {
	...
	/**
	 * 在对redis进行操作的时候，我们必要的时候可以添加锁，避免发生缓存击穿等问题
	 */
	@Autowired
	 ...
	 * @param lockKey 选择需要加锁的key
	 * @param timeout 选择所得过期时间
	 * @return 如果成功返回获取到的value，如果失败返回null
	 */
	public String get(String key, int indexdb, String lockKey, int timeout) {
		redissonUtil.lock(lockKey, timeout);
		Jedis jedis = null;
		String value = null;
		try {
			...
			returnResource(jedisPool, jedis); // 释放redis链接的链接资源
			redissonUtil.unlock(lockKey); // 无论操作是否成功都必须得释放锁资源
		}
		return value;
	}
	 ...
	 */
	public Long expire(String key, int timeout, int indexdb) {
		Jedis jedis = null;
		try {
			jedis = jedisPool.getResource();
			jedis.select(indexdb);
